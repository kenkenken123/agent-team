...
export class OfficeState {
  ...
  /** Maps "parentId:toolId" 鈫?sub-agent character ID (negative) */
  subagentIdMap: Map<string, number> = new Map();
  /** Reverse lookup: sub-agent character ID 鈫?parent info */
   ...
   * repeat in balanced rounds with a random hue shift (鈮?5掳).
   */
  private pickDiversePalette(): { palette: number; hueShift: number } {
    ...
    // First round (minCount === 0): no hue shift. Subsequent rounds: random 鈮?5掳.
    let hueShift = 0;
    if (minCount > 0) {
  ...
  addAgent(
    ...
    if (seatId) {
      ...
      // No seats 鈥?spawn at random walkable tile
      const spawn =
  ...
  reassignSeat(agentId: number, seatId: string): void {
    ...
    if (path.length > 0) {
      ...
      // Already at seat or no path 鈥?sit down
  ...
  removeSubagent(parentAgentId: number, parentToolId: string): void {
    ...
    if (ch) {
      if (ch.matrixEffect === 'despawn') {
        // Already despawning 鈥?just clean up maps
      ...
      // Start despawn animation 鈥?keep character in map for rendering
      ch.matrixEffect = 'despawn';
      ch.matrixEffectTimer = 0;
      ch.matrixEffectSeeds = matrixEffectSeeds();
      ch.bubbleType = null;
  ...
  /** Dismiss bubble on click 鈥?permission: instant, waiting: quick fade */
      ...
      if (ch.matrixEffect) {
        ...
        if (ch.matrixEffectTimer >= MATRIX_EFFECT_DURATION) {
          if (ch.matrixEffect === 'spawn') {
            // Spawn complete 鈥?clear effect, resume normal FSM
            ch.matrixEffect = null;
            ch.matrixEffectTimer = 0;
            ch.matrixEffectSeeds = [];
          } else {
            // Despawn complete 鈥?mark for deletion
            toDelete.push(ch.id);
          }
        }
