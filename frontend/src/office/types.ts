...
export interface FloorColor {
  ...
  /** Contrast -100 to 100 */
  c: number;
  /** When true, use Photoshop-style Colorize (grayscale 鈫?fixed HSL). Default: adjust mode. */
  colorize?: boolean;
}
...
export interface Character {
  ...
  /** Palette index (0-5) */
  palette: number;
  /** Hue shift in degrees (0 = no shift, 鈮?5 for repeated palettes) */
  hueShift: number;
  ...
  /** Countdown timer for bubble (waiting: 2鈫?, permission: unused) */
  bubbleTimer: number;
