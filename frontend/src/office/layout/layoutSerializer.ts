...
export function getBlockedTiles(
  ...
  for (const item of furniture) {
    ...
    for (let dr = 0; dr < entry.footprintH; dr++) {
      if (dr < bgRows) continue; // skip background rows 鈥?characters can walk through
      for (let dc = 0; dc < entry.footprintW; dc++) {
        const key = `${item.col + dc},${item.row + dr}`;
...
/** Get tiles blocked for placement purposes 鈥?skips top backgroundTiles rows per item */
...
export function layoutToSeats(furniture: PlacedFurniture[]): Map<string, Seat> {
  ...
  const dirs: Array<{ dc: number; dr: number; facing: Direction }> = [
    { dc: 0, dr: -1, facing: Direction.UP }, // desk is above chair 鈫?face UP
    { dc: 0, dr: 1, facing: Direction.DOWN }, // desk is below chair 鈫?face DOWN
    { dc: -1, dr: 0, facing: Direction.LEFT }, // desk is left of chair 鈫?face LEFT
    { dc: 1, dr: 0, facing: Direction.RIGHT }, // desk is right of chair 鈫?face RIGHT
  ];
...
export function createDefaultLayout(): OfficeLayout {
  ...
  // Minimal fallback with no furniture 鈥?the default-layout.json provides the real default
...
// 鈹€鈹€ Furniture type migration 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
const LEGACY_TYPE_MAP: Record<string, string | null> = {
  ...
  bookshelf: 'BOOKSHELF',
  plant: 'PLANT',
  cooler: null, // no equivalent in new assets 鈥?remove
  whiteboard: 'WHITEBOARD',
  pc: 'PC_FRONT_OFF',
  lamp: null, // no equivalent in new assets 鈥?remove
};
...
function migrateFurnitureTypes(furniture: PlacedFurniture[]): PlacedFurniture[] {
  ...
  for (const item of furniture) {
    ...
    if (newType === undefined) {
      // Not a legacy type 鈥?keep as-is
      migrated.push(item);
    } else if (newType !== null) {
      // Migrate to new type
      migrated.push({ ...item, type: newType });
    }
    // newType === null 鈫?remove the item (no equivalent)
  }
  return migrated;
}
...
function migrateLayout(layout: OfficeLayout): OfficeLayout {
  ...
  // Migrate old VOID value (was 8, now 255) 鈥?only for legacy layouts since FLOOR_8 reuses value 8
  const OLD_VOID = 8;
  ...
  // Check if any tiles use old values (1-4) 鈥?these map directly to FLOOR_1-4
  ...
  for (const tile of layout.tiles) {
    switch (tile) {
      case 0: // WALL
        tileColors.push(null);
        break;
      case 1: // was TILE_FLOOR 鈫?FLOOR_1 beige
        tileColors.push(DEFAULT_LEFT_ROOM_COLOR);
        break;
      case 2: // was WOOD_FLOOR 鈫?FLOOR_2 brown
        tileColors.push(DEFAULT_RIGHT_ROOM_COLOR);
        break;
      case 3: // was CARPET 鈫?FLOOR_3 purple
        tileColors.push({ h: 280, s: 40, b: -5, c: 0 });
        break;
      case 4: // was DOORWAY 鈫?FLOOR_4 tan
        tileColors.push({ h: 35, s: 25, b: 10, c: 0 });
        break;
      default:
        // Floor tile types without colors 鈥?use neutral gray
